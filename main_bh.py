...
def sigmoid_derivative(x, t, tau):
    something = sigmoid(x, t, tau)
    return (1 / tau(t)) * something * (1 - something)
...
def estimate_gamma_params(samples):
    ...
    var = np.var(samples)
    alpha_hat = mean**2 / var if var > 0 else 5.0
    beta_hat = mean / var if var > 0 else 3.0
...
def update_policy_parameters(s, S, x, x_next, w, b1, b2, t, alpha_hat, beta_hat, tau, S_tilde, phi_scale):
    ...
    if eta_S == 0: # S는 주문이 발생했을 때만 영향을 미치는 파라미터, 주문을 안했으면 S가 사용되지도 않고 학습할 필요도 없으므로 주문이 일어난 경우의 전이확률만 고려
        ...
        z_S = S - np.random.gamma(alpha_hat+1, 1 / beta_hat)
...
#    b2 = lambda t: 0.01 / (np.floor(t/10) + 1) 
    ...
    for t in range(1000):
        ...
        if t >= warmup_period:
            ...
            debug_dict["cost_A"].append(mean(cost_history))
        else:
            rho_history.append(rho)
            w_history.append(w)
            s_history.append(s)
...
def plot_debug_variables(debug_dict, s_history, S_history, config):
    ...
    plt.subplot(4, 2, 7)
    plt.plot(steps, debug_dict["cost_A"], label="cost_A")
    plt.title("Average_cost"); plt.grid(True)
